...
* Sequence numbers are in packets; start with 1
* Must perform an integrity check (XOR all bytes, must be 0)  
* Maximum Window Size of 16      packets  
* Fixed retransmission delay of **1** **second**  
* NACKs are calculated by taking the difference between the current packet number and the latest contiguous packet number 
* Must retransmit after **3** **duplicate NACK differences**
...
* Sequence Number (SEQ; 2 bytes)  
  This number represents the packet number. When sending data, the first packet number should be 1. For example, if there are 2000 bytes in stdin, the first packet (SEQ = 1) has the first 1016 bytes and the second has the last 984 bytes.

* Negative Acknowledgement Number ((N)ACK; 2 bytes)  
  The number helps notify the sender of any loss; it represents the latest contiguous packet received relative to the current packet. **If the receiver has received all packets up to the SEQ in a received packet, the (N)ACK field should be 0.** 

  However, let's say that packets 25, 28, and 29 are missing and the receiver just received packet 30. **The receiver should respond with the (N)ACK field set to 30 - (25 - 1) = 6 since 25 - 1 = 24 is the latest contiguous packet received relative to the current packet.** (Consequently, if the receiver just received packet 25, but has packets  up to 30, the (N)ACK field should be set to -5).
...
* Flags (F; 1 byte)  
  This single byte holds various packet metadata. The (P)arity flag is set to ensure data integrity. **Taking the XOR of all the bits in a packet should yield 0.** If not, this packet is invalid and should be dropped. The N(A)CK flag is set when the receiver would like to notify the sender of loss; in other words, when the NACK number is not 0, this flag should be set. 

  The P flag is the least significant bit, and the A flag is the second-least significant bit. You may access their values like so:  
...
```c
#define MSS 1016 // MSS = Maximum Segment Size (aka max length)
typedef struct {
    uint16_t seq; 
    uint16_t length;  
    uint16_t unused;  
...
uint16_t seq = ntohl(pkt->seq); // Make sure to convert to little endian
```
...
### NACK difference
From now on, we'll refer to the number SEQ - (N)ACK as the NACK difference. This represents the earliest packet that hasn't been acknowledged.

### Sending Data

Read as much as you can from standard input and place it in a new packet’s payload. Send the packet over the socket, but keep it in a *sending buffer*. Keep doing this until you’ve reached the **maximum window size** **of 16 packets**. You may only keep sending once you receive acknowledgement of packets (which reduces the amount of packets in your window). 

### Receiving Data

Once you receive a new packet, check if the N(A)CK flag is set. If it is, do a linear scan of all the packets in the sending buffer. Any packet with a SEQ less than or equal to the NACK difference is assumed to be acknowledged. You may remove them from the sending buffer. This reduces the amount of packets in your window and should let you send more packets if needed.
...
Finally, send an negative acknowledgement. You may do this by sending it with another data packet (with NACK set and flag), or you may send a dedicated NACK packet (if you have no more data to send or reached your maximum window size). Dedicated NACK packets have a SEQ of 0 and should not go in the send buffer. The NACK number you use is based on the packet you just received.

### Retransmission

After **1 second** of receiving any NACKs from the receiver (or no sort of data at all), retransmit the packet with the lowest sequence number in the sending buffer. Reset this timer as soon as you receive a packet with a new NACK difference.

After **3** packets in a row with the same NACK difference, retransmit the packet with the lowest sequence in the sending buffer. Do not reset the timer.
...
* Make sure that **every** number (SEQs, lengths, etc.) is in network order/big endian.
...
1. **Reliable Data Transport (Your Client \<-\> Your Server): Small file (1 KB): `test_self` (10 points)**  
...
2. **Reliable Data Transport (Your Client \<-\> Your Server): Medium file (50 KB), Drop (5%): `test_self_drop` (5 points)**  
...
3. **Reliable Data Transport (Your Client \<-\> Reference Server): Small file (1 KB): `test_client` (10 points)**  
   Same as 1, but uses the reference server instead.

4. **Reliable Data Transport (Reference Client \<-\> Your Server): Small file (1 KB): `test_server` (10 points)**  
   Same as 1, but uses the reference client instead.

5. **Reliable Data Transport (Your Client \<-\> Reference Server): Medium file (50 KB), Drop (5%): `test_client_drop` (10 points)**  
   Same as 2, but uses the reference server instead. **Timeout: 30 seconds.**

6. **Reliable Data Transport (Reference Client \<-\> Your Server): Medium file (50 KB), Drop (5%): `test_server_drop` (10 points)**  
   Same as 2, but uses the reference client instead. **Timeout: 30 seconds.**

7. **Reliable Data Transport (Your Client \<-\> Reference Server): Medium file (50 KB), Corrupt (5%): `test_client_int` (5 points)**  
   Same as 2, but uses the reference server instead, no drops, and randomly chooses packets to flip one bit. **Timeout: 30 seconds.**

8. **Reliable Data Transport (Reference Client \<-\> Your Server): Medium file (50 KB), Corrupt (5%): `test_server_int` (5 points)**  
   Same as 2, but uses the reference client instead, no drops, and randomly chooses packets to flip one bit. **Timeout: 30 seconds.**

9. **Reliable Data Transport (Your Client \<-\> Reference Server): Large file (500 KB), Drop (1%): `test_client_drop_large` (5 points)**  
   This test case uses a substantially larger file, so we decrease the drop rate to 1%. **Timeout: 60 seconds.**

10. **Reliable Data Transport (Reference Client \<-\> Your Server): Large file (500 KB), Drop (1%): `test_server_drop_large` (5 points)**  
   Same as 7, but uses the reference client instead. **Timeout: 60 seconds.**

11. **Reliable Data Transport (Your Client \<-\> Reference Server): Ginormous file (2 MB): `test_ginormous` (15 points)**  
