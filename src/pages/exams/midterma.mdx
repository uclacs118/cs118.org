...
Note that this would leads to a queue at \\(R1\\), but that is ok since the queue will never grow pass 3 packets as that is the maximum number of packets that is in-flight.
