---
layout: ../../layouts/Layout.astro
title: "Project 2: Reliable Data Transport"
dueDate: 2026-05-18 23:59:00
---

import * as H from "../../components/helpers/index.astro";

<H.Title>Project 2</H.Title>
<H.Subtitle>Reliable Data Transport - Due 11:59pm Monday, May 18 (Week 12)</H.Subtitle>

## Quick Links

- [Starter Code](https://github.com/uclacs118/project2s26-starter)
- [Environment Setup](/projects/environment-setup)
- [Local Autograder](/projects/autograder)

## Overview

In this project, you will implement a **reliable data transport (RDT) protocol** in C. You will implement the elements of reliable data transport you learned from class, including:

- Packet structure with sequence numbers and acknowledgements
- Checksum validation for data integrity
- Sliding window for flow control
- Retransmission logic with timeouts
- Handling of packet loss and reordering

Your implementation will communicate over UDP but provide reliable delivery to the application layer, similar to how TCP provides reliability over IP.

### Architecture

Your client and server will follow a layered architecture:

![Architecture Diagram](/projects/arch.png)

- **STDIO Layer** (`io.c`): Reads from stdin, writes to stdout
- **Logic Layer** (`transport.c`): Implements RDT protocol - packet construction, acknowledgements, retransmission, flow control
- **NetIO Layer** (`client.c` / `server.c`): Sends and receives packets over the network

## Notices

1. If you're using GitHub or a similar tool to track your work, please make sure that your repository is **set to private**.
2. As per the syllabus, the use of AI assisted tools is not allowed.
3. You may do this project in groups of up to 3 people. Please remember to include your whole group in your gradescope submission.

## Submission

Submit a ZIP file to [Gradescope](https://www.gradescope.com/courses/1292484/assignments/8055251) by **11:59pm on Monday, May 18th**. Late submission is allowed until Wednesday, May 20th (10% deduction per day).

The `helper zip` command creates a compatible ZIP file.

## Grading Criteria

The autograder runs 17 tests for a total of **100 points**:

| Test | Points |
| ---- | ------ |
| Compilation | 4 |
| Small file (10 KB) - Baseline | 6 |
| Small file (10 KB) - Drop (5%) | 6 |
| Small file (10 KB) - Reorder (100%) | 6 |
| Small file (10 KB) - Drop + Reorder | 6 |
| Medium file (100 KB) - Baseline | 6 |
| Medium file (100 KB) - Drop (5%) | 6 |
| Medium file (100 KB) - Reorder (100%) | 6 |
| Medium file (100 KB) - Drop + Reorder | 6 |
| Large file (500 KB) - Baseline | 6 |
| Large file (500 KB) - Drop (5%) | 6 |
| Large file (500 KB) - Reorder (100%) | 6 |
| Large file (500 KB) - Drop + Reorder | 6 |
| Ginormous file (2 MB) - Baseline | 6 |
| Ginormous file (2 MB) - Drop (5%) | 6 |
| Ginormous file (2 MB) - Reorder (100%) | 6 |
| Ginormous file (2 MB) - Drop + Reorder | 6 |

### Test Conditions

Each file size has four test scenarios:
- **Baseline**: No packet loss or reordering
- **Drop (5%)**: 5% of packets are dropped
- **Reorder (100%)**: All packets are reordered (sent out of order)
- **Drop + Reorder**: 5% packet loss combined with 100% reordering

The autograder compares the output byte-for-byte with the input. Your implementation must correctly:
- Sequence and acknowledge packets
- Detect corruption via checksum
- Retransmit lost packets
- Reorder buffered packets in correct order
- Manage the sliding window for flow control

### Compilation Requirements

Your submission must:
- Include a `Makefile` that builds `client` and `server` executables

### Running Tests Locally

Since running all tests may take a long time, you can run individual test categories using the helper script:

```bash
./helper test compile   # Run compilation test only
./helper test small    # Run small file (10 KB) tests only
./helper test medium   # Run medium file (100 KB) tests only
./helper test large    # Run large file (500 KB) tests only
./helper test ginormous # Run ginormous file (2 MB) tests only
```

You can also run tests by condition across all file sizes:

```bash
./helper test baseline      # All baseline tests (no loss/reorder)
./helper test drop         # All drop tests (5% packet loss)
./helper test reorder      # All reorder tests (100% reordering)
./helper test drop_reorder # All drop + reorder tests
```

Run `./helper` without arguments to see all available commands.

## Resources

- [RFC 793](https://tools.ietf.org/html/rfc793) - Transmission Control Protocol (original)
- [RFC 9293](https://www.rfc-editor.org/rfc/rfc9293.html) - Transmission Control Protocol (latest standard, August 2022)
- [Beej's Guide to Network Programming](https://beej.us/guide/bgnet/)
