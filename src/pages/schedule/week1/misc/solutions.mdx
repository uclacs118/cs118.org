...
1411200 bits per second * (3 * 60 + 35) seconds = 303408000 bits.

303408000 bits / 56000 bits per second = 5418 seconds to transfer (transmission delay).

20,000,000 m/s / 560km = 0.0028 s (propagation delay)

5418.003s = 90.30005min

90.30005min * $0.85/min = $76.75
...
8. % time wasted: It takes 24 ms to transfer to node A, then 6 ms to get to node C, since as soon as any bit arrives at B, it is immediately forwarded to C, and with no queueing due to the faster B-C link speed. This gives for a total of 30 ms if streamed. (31.6 - 30)/31.6 = 5%. Notice how even with just 3 packets, the percent of time "wasted" is quite low.
