...
As seen in the Project 3 spec, instead of using packet headers with predefined byte boundaries, we'll be using Type-Length-Value (TLV) encoding for the security messages. TLV can be a bit tricky to work with, so we're providing some TLV helpers in the `consts.h` file found in the starter code.
...
Since all these TLV objects are dynamically allocated, we need to free them to conserve memory:
...
To let the transport layer know about the data we've sent, return the length of the data:
...
Make sure that the resulting value is not `NULL`. If it is, that means the data inside `buf` does not represent a valid TLV packet. Feel free to use the `print_tlv_bytes` function to print as much of the packet as possible.
