...
Clone the **Starter Code**. The `helper` tools work exactly the same as Projects 0 and 1—see **Using the Local Autograder** and **Using the Local Autograder (…for Project 3)** for more info.

## Specification

Create two programs (a server and a client) written in C/C++ (up to version 17). Both programs will use [BSD sockets](https://en.wikipedia.org/wiki/Berkeley_sockets) using IPv4 and UDP to listen/connect. For cryptographic primitives, use `libcrypto` which is part of OpenSSL (version 3); the provided `libsecurity` does most of the heavy lifting for you. The expected behavior is the exact same as Project 2, but with different inner logic. See the Project 0 specification and Project 2 Specification for more information.
