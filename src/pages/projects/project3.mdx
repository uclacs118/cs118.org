...
7. Use `./helper submit` to create your submission ZIP.
8. A lot of material in this specification may seem foreign to you—that's okay! Make sure to review the Week 9 discussion slides which go over the intuition behind these new cryptographic protocols.
...
The certificate contains three values: the associated DNS name (e.g. example.com), a public key, and a signature of the `DNS-Name`, `Public-Key` and `Lifetime` appended after each other by the certificate authority. `Lifetime` payload has 16 bytes, the first 8 bytes contains a *notBefore* in Unix Timestamp in big endian, while the second 8 bytes contains a *notAfter*, also a Unix Timestamp in big endian. They two together describes a validity period of a certificate, and client must not accept an expired (or yet valid) certificate. You'll be able to access this certificate by reading the file `server_cert.bin` in the current working directory.
...
- Make sure your HMAC is computed over the TLV-serialized representation of you cryptographic primitives.
...
Submit a ZIP file to [Gradescope](https://www.gradescope.com/courses/1292484/assignments/8129653/) (course **1292484**, assignment **8129653**) by **11:59pm on Monday, June 1st**. Late submission is allowed until Wednesday, June 3rd (20% deduction per day).

In your ZIP file, include all your source code, `README.md` + a `Makefile` that generates two executables named `server` and `client`. **Do not include any other files; the autograder will reject submissions with those.** If you're using the starter repository, the `helper` tool has a `submit` method that automatically creates a compatible ZIP file.
