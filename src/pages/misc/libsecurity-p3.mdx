...
For Project 3, weâre using OpenSSL 3âs libcrypto for cryptographic primitives. However, directly interfacing with libcrypto may prove to be quite challenging for a lot of students. As such, weâre providing our own libsecurity that has more course-grained cryptographic primitives that are specific to Project 3.

Global Variables

EVP_PKEY* ec_peer_public_key

This variable is populated after a valid call to load_peer_public_key. Use this as the authority for verify if youâd like to check if some data was signed with the peerâs (the other client or server) private key.

EVP_PKEY* ec_ca_public_key

This variable is populated after a valid call to load_ca_public_key. Use this as the authority for verify if youâd like to check if some data was signed with the certificate authorityâs private key.

uint8_t* certificate and size_t cert_size

This variable is populated after a valid call to load_certificate. This certificate is already encoded as TLV 0xA0 (see the Project 3 spec for more info).cert_size is the corresponding total length of the certificate.

uint8_t* public_key and size_t pub_key_size

This variable is populated after a valid call to derive_public_key. This public key is encoded as ASN.1 DER; you do not need to know the specifics about this encoding. This is the format that we use to send public keys over the wire.pub_key_size is the corresponding total length of the public_key.

load_private_key

```c

<span>void load_private_key(const char* filename);</span>
...
const char* filename: The absolute or relative path to the private key. (Hint: in Project 3, itâs just server_key.bin.)

get_private_key

```c

<span>EVP_PKEY* get_private_key();</span>
...
<span>void set_private_key(EVP_PKEY* key);</span>
...
<span>void load_peer_public_key(const uint8_t* peer_key, size_t size);</span>

### ```

Take some public key sent over the wire and load it as the peerâs public key (in ec_peer_public_key).
...
<span>void load_ca_public_key(const char* filename);</span>

### ```

From a file on the local file system, load in the CAâs public key.

Parameters

const char* filename: The absolute or relative path to the CAâs public key. (Hint: in Project 3, itâs just ca_public_key.bin.)

load_certificate

```c

<span>void load_certificate(const char* filename);</span>
...
const char* filename: The absolute or relative path to the certificate. (Hint: in Project 3, itâs just server_cert.bin.)

generate_private_key

```c

<span>void generate_private_key();</span>
...
<span>void derive_public_key();</span>
...
<span>void derive_keys(const uint8_t* salt, size_t size);</span>
...
const uint8_t* salt: Buffer containing the salt material for key derivation. (Hint: For Project 3, itâs the Client-Hello with the Server-Hello appended right after.)
...
<span>size_t sign(uint8_t* signature, const uint8_t* data, size_t size);</span>
...
<span>int verify(const uint8_t* signature, size_t sig_size, const uint8_t* data, size_t size, EVP_PKEY* authority);</span>
...
<span>void generate_nonce(uint8_t* buf, size_t size);</span>
...
<span>size_t encrypt_data(uint8_t* iv, uint8_t* cipher, const uint8_t* data, size_t size);</span>
...
<span>size_t decrypt_cipher(uint8_t* data, const uint8_t* cipher, size_t size, const uint8_t* iv);</span>
...
<span>void hmac(uint8_t* digest, const uint8_t* data, size_t size);</span>
