...
3. The lost frames are retransmitted in future packets with new sequence numbers.
   - This does occur.
4. Data from stream 14 remains blocked until the lost frames are retransmitted and received.
   - Data from stream 14 is not blocked; the lost frames are only from streams 15 and 16.
5. Data from stream 15 remains blocked until the lost frames are retransmitted and received.
   - Data from stream 15 is blocked; some of the lost frames are from stream 15, and they must be delivered before more frames from the stream can be delivered.
6. Data from stream 16 remains blocked until the lost frames are retransmitted and received.
   - Data from stream 16 is blocked; some of the lost frames are from stream 16, and they must be delivered before more frames from the stream can be delivered.
