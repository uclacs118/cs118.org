...
Consider the network shown below. The costs of all links are initially: c(x,y) = 4, c(x,z) = 50, c(y,w) = 1, c(z,w) = 1, c(y,z) = 3.

![q1](/imgs/hw4p1.pdf)

1. **(8 points)** Suppose the link-state routing is used. Show the intermediate steps to compute the shortest path from x to each of the other nodes.

2. Suppose the distance-vector routing is used. 

    a. **(6 points)** When the distance vector routing is stabilized, router w, y, and z inform their distances to x to each other. What distance values do they tell each other?
