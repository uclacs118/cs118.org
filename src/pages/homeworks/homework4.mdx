...
All ASes are running OSPF for their intra-AS routing protocol.  
...
Suppose all local preferences are the same. Once router 1d learns about \\(x\\), it will put an entry \\((x, I)\\) in its forwarding table:
