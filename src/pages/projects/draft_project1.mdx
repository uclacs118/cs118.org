...
* Minimum/initial window size of 1; current window determined by receiver's Flow Window field
...
* Sequence Number (SEQ; 2 bytes)  
  This number represents the packet number. When sending data, the first packet number should be randomized (e.g. 789). For example, if there are 2000 bytes in stdin, the first packet (SEQ = 789) has the first 1016 bytes and the second (SEQ = 790) has the last 984 bytes.
...
* Flow Window (W; 1 byte)  
  This field lets the sender know the maximum flow window the receiver can handle. For example, if the receiver sends an packet with this field set to 4, the sender should not send any more than 4 unacknowledged packets at a time. **Note:** the initial window size is always 1.

* Flags (F; 1 byte)  
  This single byte holds various packet metadata. If the (S)YN flag is on, this means that this packet seeks to synchronize sequence numbers (by providing an initial one in the SEQ field). The (A)CK flag is on when the packet is carrying an acknowledgement. The (P)arity flag is set to ensure data integrity. **Taking the XOR of all the bits in a packet should yield 0.** If not, this packet is invalid and should be dropped. 
...
Note that the layout suggests that the largest packet size is 1024 bytes (or maximum length 1016 bytes). In order to read in a packet, you may do the following:
...
3. The client finally responds with another packet. This packet has the ACK flag set, the ACK number set to the server’s SEQ \+ 1, and the SEQ incremented by 1 from the very first packet. 
...
Read as much as you can from standard input and place it in a new packet’s payload. Send the packet over the socket, but keep it in a *sending buffer*. Keep doing this until you’ve reached the receiver's window specified in the Flow Window field. Note that the default window size is 1, and it will never decrease in size. You may only keep sending once you receive acknowledgement of packets (which reduces the amount of packets in your window). 
...
2. **Reliable Data Transport (Your Client \<-\> Your Server): Medium file (100 KB), Drop (5%):   
   `TestRDT.test_self_drop` (5 points)**  
...
3. **Reliable Data Transport (Your Client \<-\> Reference Server): Small file (10 KB):   
   `TestRDT.test_client` (10 points)**  
   Same as 1, but uses the reference server instead.

4. **Reliable Data Transport (Reference Client \<-\> Your Server): Small file (10 KB):   
   `TestRDT.test_server` (10 points)**  
   Same as 1, but uses the reference client instead.

5. **Reliable Data Transport (Your Client \<-\> Reference Server): Medium file (100 KB), Drop (5%):   
   `TestRDT.test_client_drop` (10 points)**  
   Same as 2, but uses the reference server instead. **Timeout: 30 seconds.**

6. **Reliable Data Transport (Reference Client \<-\> Your Server): Medium file (100 KB), Drop (5%):   
   `TestRDT.test_server_drop` (10 points)**  
   Same as 2, but uses the reference client instead. **Timeout: 30 seconds.**

7. **Reliable Data Transport (Your Client \<-\> Reference Server): Medium file (100 KB), Corrupt (5%):   
   `TestRDT.test_client_int` (5 points)**  
   Same as 2, but uses the reference server instead, no drops, and randomly chooses packets to flip one bit. **Timeout: 30 seconds.**

8. **Reliable Data Transport (Reference Client \<-\> Your Server): Medium file (100 KB), Corrupt (5%):   
   `TestRDT.test_server_int` (5 points)**  
   Same as 2, but uses the reference client instead, no drops, and randomly chooses packets to flip one bit. **Timeout: 30 seconds.**

9. **Reliable Data Transport (Your Client \<-\> Reference Server): Large file (500 KB), Drop (1%):   
...
10. **Reliable Data Transport (Reference Client \<-\> Your Server): Large file (500 KB), Drop (1%):   
    `TestRDT.test_server_drop_large` (5 points)**  
    Same as 7, but uses the reference client instead. **Timeout: 60 seconds.**

11. **Reliable Data Transport (Your Client \<-\> Reference Server): Ginormous file (2 MB):   
    `TestRDT.test_ginormous` (15 points)**  
