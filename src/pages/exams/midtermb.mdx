...
title: "Midterm (Version B)"
---

# Question 1: Bandwidth and Delay (25 points)

![problem](/imgs/midbp1.svg)

Host \\(A\\) continuously sends **4** packets to Host \\(B\\) via 2 routers \\(R_1\\) and \\(R_2\\).
- The packet length is \\(250\\;\rm bytes\\) each.
- The \\(A-R_1\\) and \\(R_2-B\\) links each have \\(8\\;\rm Mbps\\) of bandwidth and \\(2\\;\rm ms\\) of propagation delay.
- The \\(R_1-R_2\\) link has \\(2\\;\rm Mbps\\) of bandwidth, \\(4\\;\rm ms\\) of propagation delay.
...
the time gap between receiving the last bit of the first packet and the last bit of 3rd packet).
    ...
    Referencing the table, we see that 9.25 ms - 7.25 ms = **2 ms**.

2. **(7 points)** What is the time gap between the first and third packets when they arrive at Host \\(B\\)?

    Referencing the table, we see that 11.5 ms - 9.5 ms = **2 ms**.

3. **(11 points)** When will Host \\(B\\) receive all 4 packets?

    Referencing the table, we see that packet 4 completely arrives at \\(B\\) at **12.5 ms**.

\\(A-R_1\\) and \\(R_2-B\\) transmission time for one packet: \\(0.25\\;\rm ms\\)  
\\(R_1-R_2\\) transmission time for one packet: \\(1\\;\rm ms\\)

|Packet|Leaving A|Arriving R1|Leaving R1|Arriving R2|Leaving R2|Arriving B|
|---   |---      |---        |---       |---        |---       |---       |
|1     |0.25ms   |2.25ms     |3.25ms    |7.25ms     |7.5ms     |9.5ms     |
|2     |0.5ms    |2.5ms      |4.25ms    |8.25ms     |8.5ms     |10.5ms    |
|3     |0.75ms   |2.75m      |5.25ms    |9.25ms     |9.5ms     |11.5ms    |
|4     |1ms      |3ms        |6.25ms    |10.25ms    |10.5ms    |12.5ms    |

# Question 2: Utilization (25 points)
...
- For a \\(1000\\;\rm byte\\) packet to arrive at \\(B\\), it takes:
    - \\(1\\;\rm ms\\) to transmit across the \\(A-R_1\\) link (and \\(2\\;\rm ms\\) propagation time).
    - \\(4\\;\rm ms\\) to transmit across the \\(R_1-R_2\\) link (and \\(4\\;\rm ms\\) propagation time).
    - \\(1\\;\rm ms\\) to transmit across the \\(R_2-B\\) link (and \\(2\\;\rm ms\\) propagation time).
- For the acknowledgement to return to \\(A\\), we add up all the propagation times: \\(2\\;\mathrm\{ms\} + 4\\;\mathrm\{ms\} + 2\\;\mathrm\{ms\} = 8\\;\mathrm\{ms\}\\)

Adding up all these times, it takes \\(1\\;\mathrm\{ms\}+4\\;\mathrm\{ms\}+1\\;\mathrm\{ms\}+8\\;\mathrm\{ms\}+8\\;\mathrm\{ms\} = 22\\;\mathrm\{ms\}\\) to send one packet and receive its acknowlegement. 

During this time, we spend only \\(1\\;\mathrm\{ms\}\\) transmitting a packet across the \\(A-R_1\\) link. This results in \\(1 \\;\mathrm\{ms\} / 22 \\;\mathrm\{ms\} \approx \\) **4.5% utilization**.
...
To send at maximum throughput, we want the bottleneck \\(R_1-R_2\\) link to be at 100 percent utilization. Packets at \\(R_1\\)'s receiving buffer leave bounded by the transmission speed of the \\(R_1-R_2\\) link. By limiting \\(N\\) to send as many packets across this link within a packet round (the interval between sending the first bit of a packet and receiving its acknowledgement), we can send at the maximum throughput (with no packet losses).

From the previous part, a packet round is \\(22\\;\rm ms\\), and the transmission time across the \\(R_1-R_2\\) link is \\(4\\;\rm ms\\).

\\(22 \\;\mathrm\{ms\} / 4 \\;\mathrm\{ms\} = 5.5\\) and rounding up yields a minimum of **6 packets**.

Note that this would leads to a queue at \\(R1\\), but that is ok since the queue will never grow pass 6 packets as that is the maximum number of packets that is in-flight.

# Question 3: HTTP + TCP (25 points)
...
- The RTT between your laptop and your cache resolver is \\(5\\;\rm ms\\). The RTT between the cache
resolver and any name server is \\(25\\;\rm ms\\).
...
receive the final answer?
    ...
    1. Firstly, we ask the caching resolver to resolve `cs118.org`. **(2.5 ms)**

    2. The caching resolver asks the root servers for the A record of `cs118.org`. It receives the NS record and the A record for the `org` nameserver. **(25 ms)**

    3. It now asks the `org` nameserver for the A record of `cs118.org`. It only receives the NS record for `cs118.org`, which is `ns1.cloudflare.com`. **(25 ms)**

    4. The caching resolver must now resolve `ns1.cloudflare.com`. It asks the root servers for the A record of `ns1.cloudflare.com`, and receives both the NS and A records for the `com` nameserver. **(25 ms)**

    5. It asks the `com` nameserver for the A record of `ns1.cloudflare.com`. It receives the NS and A records for the `cloudflare.com` nameserver, which happens to be `ns1.cloudflare.com`! **(25 ms)**

    6. It can now ask this nameserver for the A record of `cs118.org`, which yields the final result. **(25 ms)**

    7. The caching resolver returns this result to us. **(2.5 ms)**

    **Answer: 130 ms** (or \\(T=0.130\\;\rm s\\))

2. **(6 points)** At \\(T = 30\\;\rm s\\), your laptop queries the **A** record of `eado.me`. When will your laptop
receive the final answer?

    1. Firstly, we ask the caching resolver to resolve `eado.me`. **(2.5 ms)**

    2. The caching resolver asks the root servers for the A record of `eado.me`. It receives the NS record and the A record for the `me` nameserver. **(25 ms)**

    3. It now asks the `me` nameserver for the A record of `eado.me`. It only receives the NS record for `eado.me`, which is `ns1.cloudflare.com`. **(25 ms)**

    4. The caching resolver already knows the A record of `ns1.cloudflare.com` (TTL: \\(30\\;\rm s\\)). It asks it for the A record of `eado.me` and receives it. **(25 ms)**

    5. The caching resolver returns this result to us. **(2.5 ms)**

    **Answer: 80 ms** (or \\(T=30.080\\;\rm s\\))
...
3. **(6 points)** At \\(T = 610\\;\rm s\\), your laptop queries the **A** record of `eado.me` again. When will your laptop receive the final answer?
    ...
    2. The RRs for `eado.me` have expired, so the caching resolver must resolve it. However, it already knows the A record of `me`'s nameserver (TTL: \\(20\\;\rm s\\)), so it asks it for the A record of `eado.me`. It only receives the NS record for `eado.me`, which is `ns1.cloudflare.com`. **(25 ms)**

    3. Unfortunately, the `com` nameserver RRs have since expired, so the caching resolver must now resolve `ns1.cloudflare.com` (which has also expired) from the start. It asks the root servers for the A record of `ns1.cloudflare.com`, and receives both the NS and A records for the `com` nameserver. **(25 ms)**

    4. It asks the `com` nameserver for the A record of `ns1.cloudflare.com`. It receives the NS and A records for the `cloudflare.com` nameserver, which happens to be `ns1.cloudflare.com`. **(25 ms)**

    5. It can now ask this nameserver for the A record of `eado.me`, which yields the final result. **(25 ms)**

    6. The caching resolver returns this result to us. **(2.5 ms)**

    **Answer: 105 ms** (or \\(T=610.105\\;\rm s\\))

4. **(6 points)** At \\(T = 680\\;\rm s\\), your laptop queries the **A** record of `elamri.me`. When will your laptop receive the final answer?

    1. Firstly, we ask the caching resolver to resolve `elamri.me`. **(2.5 ms)**

    2. Unfortunately, the `me` nameserver RRs have expired, so the caching resolver must resolve `elamri.me` from the start. It asks the root servers for the A record of `elamri.me`, and receives both the NS and A records for the `me` nameserver. **(25 ms)**

    3. It asks the `me` nameserver for the A record of `elamri.me`. It only receives the NS record for `elamri.me`, which is `ns1.cloudflare.com`. **(25 ms)**

    4. The RRs for `ns1.cloudflare.com` have since expired, so the caching resolver must resolve it. However, it already knows the A record of `com`'s nameserver (TTL: \\(530\\;\rm s\\)). It asks the `com` nameserver for the A record of `ns1.cloudflare.com`. It receives the NS and A records for the `cloudflare.com` nameserver, which happens to be `ns1.cloudflare.com`. **(25 ms)**

    5. It can now ask this nameserver for the A record of `elamri.me`, which yields the final result. **(25 ms)**

    6. The caching resolver returns this result to us. **(2.5 ms)**

    **Answer: 105 ms** (or \\(T=680.105\\;\rm s\\))
