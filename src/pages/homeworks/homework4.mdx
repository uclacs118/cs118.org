...
1. **(9 points)** When the distance vector routing is stabilized, router w, y, and z inform their distances to x to each other. What distance values do they tell each other?
2. **(9 points)** Now suppose that the link cost between x and y increases to 60. Can y, z, w settle their optimal distances to x within several (say, \\(\le5\\)) iterations? Why or why not?
3. **(7 points)** Will the distance tables eventually reach a stabilized state? Why or why not?s
...
1. Assume that routing tables are properly configured and the network just started (i.e., all ARP caches and switch tables are empty), fill the following table to enumerate Ethernet frames (in chronological order) needed for node B to send an IP packet to 192.168.0.2 and receive a response back.

2. Assume that the previous operation is done,  fill the following table to enumerate Ethernet frames (in chronological order) for node B to send a packet to 192.168.1.253 and receive a reply.