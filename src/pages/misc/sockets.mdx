...
title: "Socket Programming Tips - Spring 2026"
---

import * as H from "../../components/helpers/index.astro";

<H.Title>Socket Programming Tips</H.Title>
...
4. Create a buffer to store messages from clients. Also, create a temporary client address struct to inspect client information (IP address, source port).
...
When using `recvfrom`, the default behavior is to block the current thread. However, you'll want these functions to simply check if there's any data available from the other host and continue to process other data (**maybe read from standard input—which you can also make non-blocking**).
...
5. Listen for new clients by marking this socket as passive.
...
6. Accept the client connection by creating a new socket for this client. This will block the current thread until a client attempts to connect.
...
8. Listen to messages from the client.
...
9. Send a message back to the client.
...
6. Listen to messages from server.
