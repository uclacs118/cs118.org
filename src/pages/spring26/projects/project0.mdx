...
title: "Project 0: Environment Setup & Socket Programming"
dueDate: 2026-04-13 23:59:00
---

import * as H from "../../../components/helpers/index.astro";

<H.Title>Project 0</H.Title>
<H.Subtitle>Due 11:59pm Monday, Apr 13 (Week 3)</H.Subtitle>

## Quick Links

- [Starter Code](https://github.com/uclacs118/project0s26-starter)
- [Environment Setup](/spring26/projects/environment-setup)
- [Local Autograder](/spring26/projects/autograder)
- [Socket Programming Tips](/spring26/projects/sockets)
...
To get started, please review the [Environment Setup](/spring26/projects/environment-setup) and [Using the Local Autograder](/spring26/projects/autograder) pages.
...
|-----------------|-----------------|
| `host1:~/project0 $ nc -4u localhost 8080` | `host2:~/project0 $ nc -4ul 8080` |
| `> Message 1` | |
| `Message 2` | `> Message 2` |
| `> Message 3` | `Message 3` |
| `Message 4` | `> Message 4` |
...
(Lines starting with > represent input into standard input. The extra lines between the messages are to show which messages happen in what sequence.)

Your goal is to create two programs (aptly named client and server) that do the exact same thing as the example above:

| Host 1 (Client) | Host 2 (Server) |
|-----------------|-----------------|
| `host1:~/project0 $ ./client localhost 8080` | `host2:~/project0 $ ./server 8080` |
| `> Message 1` | |
| `Message 2` | `> Message 2` |

## Specification

We're now ready to start your first 118 project! If you haven't already, please clone the [Starter Code](https://github.com/uclacs118/project0s26-starter).

Create two programs written in C/C++ (up to version 17). Both programs will use [BSD sockets](https://en.wikipedia.org/wiki/Berkeley_sockets) using IPv4 and UDP to listen/connect. While you may make both programs multi-threaded, it's not recommended. Use **non-blocking file descriptors** to help.

### Libraries

Our [Socket Programming Tips](/projects/sockets) will show you how to use the socket APIs in the standard way. No other third party libraries are allowed.
...
- The `Makefile` is designed for running using the `helper` script. Use `helper interactive` instead to manually test your code.
...
1. **Data Transport (Your Client \<-\> Your Server): Small, ASCII only file (10 KB) - `test_self_ascii` (10 points)**
...
2. **Data Transport (Your Client \<-\> Your Server): Small file (10 KB) - `test_self` (10 points)**
   Same as 1, but could be any sort of data (just completely random bytes).

3. **Data Transport (Your Client \<-\> Reference Server): Small file (10 KB) - `test_client_normal` (20 points)**
   Same as 2, but uses the reference server instead.

4. **Data Transport (Your Client \<-\> Reference Server): Small file (10 KB) - `test_client_only` (20 points)**
   Same as 3, but inputs only 1 byte in the reference server.

5. **Data Transport (Reference Client \<-\> Your Server): Small file (10 KB) - `test_server_normal` (20 points)**
   Same as 2, but uses the reference client instead.

6. **Data Transport (Reference Client \<-\> Your Server): Small file (10 KB) - `test_server_only` (20 points)**
